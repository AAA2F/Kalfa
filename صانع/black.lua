...
if Dev(msg) then
if text == "تحديث" or text == "اعادة التشغيل ➢" then
LuaTele.sendText(Sudo_Id,0,"➢ تمت اعادة تشغيل الملفات بنجاح ✅")
dofile('black.lua')  
return false 
end
...
{
{text = '➢ الاحصائيات',type = 'text'},{text = '➢ الاسكرينات المفتوحة',type = 'text'},
},
{
{text = '➢ البوتات الوهمية',type = 'text'},{text = '➢ حذف البوتات الوهمية',type = 'text'},
},
{
...
{
{text = '➢ اذاعة',type = 'text'},{text = '➢ اذاعة بالتحويل',type = 'text'},
},
{
{text = '➢ تحديث المصنوعات',type = 'text'},{text = '➢ تشغيل البوتات',type = 'text'},
},
{
{text = 'اعادة التشغيل ➢',type = 'text'},
},
{
...
send(msg.chat_id,msg.id,'➢ تم حفظ القناة ',"md",true)  
Redis:del(black.."chfalse")
Redis:set(black.."chfalse",ch)
Redis:del(black.."ch:admin")
Redis:set(black.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'➢ المعرف خطأ او البوت ليس مشرف في القناة ',"md",true)  
end
end
if text == "➢ تفعيل الاشتراك الاجباري" then
Redis:set(black.."ch:addd"..msg.sender.user_id,"on")
send(msg.chat_id,msg.id,'➢ ارسل الان معرف القناة ',"md",true)  
end
if text == "➢ تعطيل الاشتراك الاجباري" then
Redis:del(black.."ch:admin")
Redis:del(black.."chfalse")
send(msg.chat_id,msg.id,'➢ تم حذف القناة ',"md",true)  
end
...
send(msg.chat_id,msg.id,"➢ اليوزر لقناة او مجموعة تأكد منه","md",true)  
return false
end
...
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعة","md",true)  
return false 
end 
Redis:set(black.."3z:gp",text)
Redis:del(black.."all:texting")
send(msg.chat_id,msg.id,"➢ جاري عمل الاذاعة لكل البوتات ومجموعاتهم يرجي الانتظار ...","html",true)
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
Redis:del(black.."all:texting")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعة في كل البوتات","html",true)
end
...
Redis:del(black.."all:texting:pv")
send(msg.chat_id,msg.id,"➢ جاري عمل الاذاعة لكل البوتات ومطورينهم ومشتركينهم يرجي الانتظار ...","html",true)
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
-------screen -ls
if text == "➢ الاسكرينات المفتوحة" then  
rqm = 0
...
send(msg.chat_id,msg.id,"➢ النمر ["..name.."](tg://user?id="..id..")\n➢ تم حظره عام","md",true)
end
...
--------mange bots
if text == "➢ حذف البوتات الوهمية" then
...
if text == "➢ البوتات الوهمية" then
local txx = "قائمة بوتاتك الوهيمه \n"
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(black..msg.sender.user_id.."make:bot")
send(msg.chat_id,msg.id, "\n➢ تم الغاء تعيين قناة السورس","md",true)  
return false 
end 
Redis:del(black..msg.sender.user_id.."make:bot")
...
send(msg.chat_id,msg.id,"➢ تم تعيين قناة البوت بنجاح")
Redis:del(black..msg.sender.user_id.."setchannel")
else
...
send(msg.chat_id,msg.id,"➢ ارسل الان معرف القناة")
return false 
end
...
return send(msg_chat_id,msg_id,'➢ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخة الحقيقيه')
end -- end Namefile
...
File:write(Get_Json)
File:close()
return LuaTele.sendDocument(msg.chat_id,msg.id,'./'..UserBot..'.json', '*• تم جلب النسخة الاحتياطية\n• تحتوي على 0 قروب \n• وتحتوي على {'..#UsersBot..'} مشترك *\n', 'md')
end
----brodcast all
...
send(msg.chat_id,msg.id,"➢ تمت الاذاعة الى *- "..#list.." * عضو في البوت ","md",true)      
Redis:del(black..msg.sender.user_id.."brodcast") 
return false
end
if text == "➢ اذاعة" then
Redis:set(black..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"➢ ارسل الاذاعة الان")
return false 
end
---fwd
...
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعة بالتحويل","md",true)    
return false 
end 
if msg.forward_info then 
local list = Redis:smembers(black.."total") 
send(msg.chat_id,msg.id,"➢ تم التحويل الى *- "..#list.." * مشترك ف البوت ","md",true)      
for k,v in pairs(list) do  
...
if text == "➢ اذاعة بالتحويل" then
Redis:set(black..msg.sender.user_id.."brodcast:fwd",true)
send(msg.chat_id,msg.id,"➢ ارسل التحويل الان")
return false 
end
...
send(msg.chat_id,msg.id,"➢ مرحبا عزيزي في مصنع الحماية لسورس الخليفة \n➢ مبرمج السورس @AAA2F","html",true, false, false, true, reply_markup)
return false 
end
---making user
...
----
if text == 'مطور السورس' or text == 'مبرمج السورس' or text == 'حسين ستار' then  
local UserId_Info = LuaTele.searchPublicChat("AAA2F")
...
{
{text = 'حسين ستار', url ="https://t.me/AAA2F"},
},
}
local msg_id = msg.id/2097152/0.5 
...
photo = "https://t.me/ccwcm/3"
local T =[[
𝐷𝐸𝑉 >> [- ᴀʟᴋʜᴀʟɪғᴀ .](t.me/AAA2F)
ـــــــــــــــــــــــــــــــــــــــ 
𝐷𝐸𝑉 >> [sᴏᴜʀᴄᴇ ᴀʟᴋʜᴀʟɪғᴀ](t.me/AAA2F)
ـــــــــــــــــــــــــــــــــــــــــــ 
 >> [sᴏᴜʀᴄᴇ ᴜᴘᴅᴀᴛᴇs](t.me/e9uuu)
ـــــــــــــــــــــــــــــــــــ
]]
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = 'حسين ستار'', url = 't.me/AAA2F'}, 
},
}
